...
void KTextPluginInstance::getSelectionLines(QString objectName)
{
  ...
  m_pEngine->globalObject().setProperty(objectName, objectValue); //TODO use return
}
...
void KTextPluginInstance::addKrossObject()
{
  //Kross::Action* action = new Kross::Action(0 /*no parent*/, KUrl(scriptfile));
  //TestKross1*  anObject = new TestKross1(action);
